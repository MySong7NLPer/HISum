...
class MarginRankingLoss(LossBase):      
    ...
    def get_loss(self, score, summary_score):
        ...
        loss_func = torch.nn.MarginRankingLoss(0.01)
        ...
        neg_score = neg_score.contiguous().view(-1)
        ones = torch.ones(pos_score.size()).cuda(score.device)
        loss_func = torch.nn.MarginRankingLoss(0.001)
        TotalLoss += loss_func(pos_score, neg_score, ones)
        ...
        #device = torch.device("cuda", TotalLoss.get_device())

        #self.i_matrix = torch.eye(loss_o.shape[-2]).to(device)
        #final_loss_o = self.mse(loss_o, self.i_matrix.unsqueeze(0).repeat(loss_o.shape[0], 1, 1))
